...
class CuriosityModule(ObjectiveModule):
    ...
    EXPLORATION_SATURATION_POINT = 1000  # 饱和点（可配置）
    MAX_PREDICTION_ERRORS = 500  # 预测误差历史最大长度（防止内存泄漏）
    ...
    def evaluate(self, state: SystemState) -> float:
        ...
        # 探索饱和度（探索越少，好奇心越高）
        exploration_saturation = len(self.exploration_history) / self.EXPLORATION_SATURATION_POINT
    ...
    def record_prediction_error(self, error: float):
        ...
        if len(self.prediction_errors) > self.MAX_PREDICTION_ERRORS:
            self.prediction_errors = self.prediction_errors[-self.MAX_PREDICTION_ERRORS:]
