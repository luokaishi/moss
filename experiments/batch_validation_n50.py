...
def run_single_experiment(run_id: int, seed: int, output_dir: Path) -> Dict:
    """运行单次实验"""
    print(f"🧪 运行实验 #{run_id + 1}/50 (seed={seed})")
    ...
    try:
        ...
        result = {
            'run_id': run_id,
            'seed': seed,
            'success': len(experiment.emergence_events) >= 3,
            'n_emergence_events': len(experiment.emergence_events),
            'emergence_events': [
                {
                    'cycle': e.cycle,
                ...
                for e in experiment.emergence_events
            ],
            'error': None
        }
...
def main():
    print("🚀 OEF 2.0 - 批量统计验证实验 (N=50, 500周期)")
    print("=" * 60)
    print("⚠️  预计时间: 20-30分钟")
    print("=" * 60)
    
    n_runs = 50
    output_dir = Path("oef_real_data/batch_validation_n50")
