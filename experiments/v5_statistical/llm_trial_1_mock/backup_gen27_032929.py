...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1.2864 for a in self.action_history[-20:] if a != 'survive')) / 20.0
            if recent_success_rate > 0.5724:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4364, 0.0992, 0.3412, 0.1233]), 'concerned': np.array([0.3982, 0.2363, 0.2521, 0.1135]), 'normal': np.array([0.3581, 0.4, 0.5856, 0.1]), 'growth': np.array([0.3226, 0.1885, 0.3425, 0.1464])}
