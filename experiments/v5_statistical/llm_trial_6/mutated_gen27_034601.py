...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1.0311 for a in self.action_history[-20:] if a != 'survive')) / 20.0
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4497, 0.2042, 0.227, 0.1192]), 'concerned': np.array([0.35, 0.35, 0.206, 0.1]), 'normal': np.array([0.1847, 0.3719, 0.3545, 0.089]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
