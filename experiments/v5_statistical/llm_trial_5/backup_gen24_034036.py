...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4227, 0.1, 0.2, 0.2053]), 'concerned': np.array([0.256, 0.35, 0.2, 0.1]), 'normal': np.array([0.2, 0.5048, 0.3, 0.1]), 'growth': np.array([0.1898, 0.1728, 0.3765, 0.1845])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'preserve', 'delegate', 'reflect']
