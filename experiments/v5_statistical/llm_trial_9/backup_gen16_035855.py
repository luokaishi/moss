...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2877, 0.2419, 0.1859, 0.2845]), 'concerned': np.array([0.35, 0.3563, 0.2, 0.1]), 'normal': np.array([0.2, 0.4, 0.349, 0.1]), 'growth': np.array([0.2, 0.2, 0.5375, 0.1826])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'cooperate']
