...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a != 'survive')) / 25.7154
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1, 0.7, 0.1, 0.1]), 'concerned': np.array([0.4154, 0.35, 0.2, 0.1]), 'normal': np.array([0.3281, 0.2838, 0.2193, 0.1688]), 'growth': np.array([0.307, 0.2023, 0.2835, 0.2072])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'create', 'delegate', 'optimize']
