...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a == 'survive')) / 18.5513
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1801, 0.4386, 0.1841, 0.1972]), 'concerned': np.array([0.3092, 0.2256, 0.2838, 0.1814]), 'normal': np.array([0.2, 0.3318, 0.346, 0.1]), 'growth': np.array([0.1786, 0.405, 0.3047, 0.262])}
