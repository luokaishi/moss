...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a != 'survive')) / 14.2925
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.6, 0.1, 0.2, 0.1]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.2, 0.3318, 0.346, 0.1]), 'growth': np.array([0.1721, 0.2611, 0.3047, 0.262])}
