...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1336, 0.5385, 0.1843, 0.1437]), 'concerned': np.array([0.262, 0.3104, 0.1895, 0.2381]), 'normal': np.array([0.2, 0.4, 0.3, 0.1]), 'growth': np.array([0.3379, 0.1683, 0.3565, 0.1373])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'delegate']
