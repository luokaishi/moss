...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.9486 for a in self.action_history[-20:] if a != 'survive')) / 20.0
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1586, 0.5013, 0.2141, 0.1259]), 'concerned': np.array([0.0297, 0.1805, 0.2768, 0.2794]), 'normal': np.array([0.0617, 0.4677, 0.3172, 0.1533]), 'growth': np.array([0.2, 0.2374, 0.1917, 0.0477])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'preserve', 'reflect']
