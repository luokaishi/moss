...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1.2345 for a in self.action_history[-20:] if a == 'survive')) / 20.0
            if recent_success_rate > 0.6022:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1547, 0.1882, 0.5684, 0.0887]), 'concerned': np.array([0.2842, 0.2501, 0.1442, 0.3215]), 'normal': np.array([0.1686, 0.3863, 0.2989, 0.1462]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
