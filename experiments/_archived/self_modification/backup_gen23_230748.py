...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.5258, 0.1811, 0.274, 0.0815]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.1653, 0.4, 0.3, 0.1]), 'growth': np.array([0.2074, 0.1851, 0.3204, 0.2871])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['survive', 'delegate', 'reflect']
