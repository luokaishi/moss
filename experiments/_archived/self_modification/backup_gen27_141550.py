...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a != 'survive')) / 29.0014
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1, 0.1, 0.1, 0.7]), 'concerned': np.array([0.4092, 0.35, 0.2549, 0.1]), 'normal': np.array([0.1948, 0.3595, 0.3077, 0.138]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
