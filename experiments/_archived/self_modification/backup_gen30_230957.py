...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a != 'survive')) / 19.6693
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.5, 0.2, 0.2, 0.1]), 'concerned': np.array([0.1035, 0.2553, 0.2, 0.1]), 'normal': np.array([0.1579, 0.4, 0.3, 0.1]), 'growth': np.array([0.282, 0.1581, 0.3562, 0.2036])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'create']
