...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a != 'survive')) / 19.6428
            if recent_success_rate > 0.6996:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.3649, 0.3462, 0.1842, 0.1048]), 'concerned': np.array([0.2714, 0.35, 0.2, 0.3083]), 'normal': np.array([0.2679, 0.3922, 0.2248, 0.1151]), 'growth': np.array([0.2261, 0.2, 0.513, 0.2])}
