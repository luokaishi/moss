...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.941 for a in self.action_history[-20:] if a == 'survive')) / 17.3
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2109, 0.2549, 0.2193, 0.2725]), 'concerned': np.array([0.3652, 0.2607, 0.1951, 0.179]), 'normal': np.array([0.1653, 0.4, 0.3, 0.1]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
