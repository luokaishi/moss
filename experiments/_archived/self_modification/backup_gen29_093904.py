...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.25, 0.25, 0.25, 0.25]), 'concerned': np.array([0.3159, 0.3564, 0.2369, 0.0909]), 'normal': np.array([0.1366, 0.4, 0.3, 0.1]), 'growth': np.array([0.2836, 0.2164, 0.4, 0.2])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['adapt', 'learn', 'reflect']
