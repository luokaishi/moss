...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1.2147 for a in self.action_history[-20:] if a != 'survive')) / 20.0
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.6, 0.1, 0.2, 0.1]), 'concerned': np.array([0.3547, 0.3137, 0.1807, 0.1508]), 'normal': np.array([0.1359, 0.3083, 0.3355, 0.2203]), 'growth': np.array([0.1774, 0.2, 0.4, 0.1447])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'maintain', 'reflect', 'preserve']
