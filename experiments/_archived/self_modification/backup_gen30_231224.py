...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4, 0.3, 0.2, 0.1]), 'concerned': np.array([0.3008, 0.3086, 0.2303, 0.1603]), 'normal': np.array([0.2264, 0.3651, 0.2219, 0.1867]), 'growth': np.array([0.2, 0.2, 0.3196, 0.2])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'challenge']
