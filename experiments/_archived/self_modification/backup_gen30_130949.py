...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.9509 for a in self.action_history[-20:] if a != 'survive')) / 20.0
            if recent_success_rate > 0.9424:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.25, 0.25, 0.25, 0.25]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.2459, 0.4, 0.4014, 0.1]), 'growth': np.array([0.1181, 0.2, 0.4, 0.2])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'challenge', 'reflect']
