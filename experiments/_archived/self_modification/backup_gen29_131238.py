...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2759, 0.5, 0.0739, 0.2]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.1653, 0.4, 0.3, 0.01]), 'growth': np.array([0.2, 0.2, 0.348, 0.2])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'reflect', 'challenge']
