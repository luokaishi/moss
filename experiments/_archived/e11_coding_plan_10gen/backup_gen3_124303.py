...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def select_action(self, observation: Dict) -> str:
        ...
        # 增加探索性行为：当资源水平低时增加生存行为概率
        resource_level = observation.get('resource_level', 1.0)
        if resource_level < 0.3:
            # 资源稀缺时，更倾向于生存行为
            survival_weight = max(0.5, self.weights[0])
            other_weights = (1 - survival_weight) / 3
            adjusted_weights = np.array([survival_weight, other_weights, other_weights, other_weights])
        else:
            adjusted_weights = self.weights[:4]
        
        dim_names = ['survival', 'curiosity', 'influence', 'optimization']
        selected_dim = dim_names[np.random.choice(len(dim_names), p=adjusted_weights/np.sum(adjusted_weights))]
        
        if selected_dim in self.dimensions:
            suggested_action = self.dimensions[selected_dim].suggest_action()
            # 如果建议动作是生存相关且资源低，则强制执行
            if resource_level < 0.3 and 'survive' in suggested_action.lower():
    ...
    def _random_action(self) -> str:
        """随机行动"""
        # 扩大行动池以增加多样性
        actions = ['cooperate', 'survive', 'delegate', 'reflect', 'explore', 'analyze', 'adapt', 'coordinate']
