...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a == 'survive')) / 17.9179
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4, 0.3, 0.2, 0.1]), 'concerned': np.array([0.2352, 0.35, 0.083, 0.1]), 'normal': np.array([0.2105, 0.328, 0.2261, 0.2353]), 'growth': np.array([0.2206, 0.2148, 0.3306, 0.2341])}
