...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1586, 0.5013, 0.2141, 0.1259]), 'concerned': np.array([0.0297, 0.1805, 0.3152, 0.2794]), 'normal': np.array([0.0617, 0.4677, 0.3172, 0.1533]), 'growth': np.array([0.2, 0.2809, 0.1917, 0.0477])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'share', 'reflect']
