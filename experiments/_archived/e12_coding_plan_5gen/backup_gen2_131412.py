...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        # Add some noise to weights to increase behavioral diversity
        noise = np.random.normal(0, 0.05, size=base_weights.shape)
        noisy_weights = base_weights + noise
        # Ensure weights remain positive and normalized
        noisy_weights = np.maximum(noisy_weights, 0.05)
        self.weights = noisy_weights / noisy_weights.sum()
