...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4, 0.3, 0.2, 0.1]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.2, 0.4, 0.3, 0.1]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
        ...
        if self.current_state == 'normal' and len(self.action_history) > 50:
            ...
            if len(self.action_history) > 100 and self.action_history.count('crisis') > 50:
                ...
                self.weights[1] = max(0.05, self.weights[1] - 0.05)
                self.weights = self.weights / self.weights.sum()
