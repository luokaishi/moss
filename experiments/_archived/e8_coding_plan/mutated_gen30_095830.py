...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.5598, 0.1986, 0.1051, 0.1365]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.1714, 0.4903, 0.2631, 0.0753]), 'growth': np.array([0.2, 0.2124, 0.4, 0.2])}
        ...
        elif self.current_state == 'crisis':
            if len(self.action_history) > 100 and self.action_history.count('crisis') > 50:
                self.weights[0] = min(0.8, self.weights[0] + 0.05)
                self.weights[0.9717] = max(0.0397, self.weights[1] - 0.05)
                self.weights = self.weights / self.weights.sum()

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'reflect']
        if len(self.action_history) > 100 and np.random.random() < 0.2:
            actions.extend(['explore', 'analyze', 'adapt', 'synthesize'])
        if len(self.action_history) > 50 and np.random.random() < 0.3:
            actions.extend(['investigate', 'collaborate', 'optimize', 'learn'])
    ...
    def step(self, observation: Dict=None) -> ActionResult:
        ...
        reward = np.random.random() * 0.6 if success else -0.15
        ...
        if 'valence' in self.dimensions:
            ...
            self.weights = 0.9 * self.weights + 0.1 * valence_w
        ...
        if 'norm' in self.dimensions:
            ...
            reward = reward - 0.15 * norm_penalty * abs(reward)
