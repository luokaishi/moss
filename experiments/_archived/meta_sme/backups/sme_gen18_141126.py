...
class PurposeGuidedSelector:
    ...
    def __init__(self, temperature: float=1.5, exploration_bonus: float=0.0953):
        """
        Args:
        ...
        for mut_type, vec in self.MUTATION_SEMANTICS.items():
            ...
            self._normalized_semantics[mut_type] = vec / (norm + 0.01)
...
class EmergenceGuidedFitness:
    ...
    def evaluate(self, agent_module, steps: int=300, purpose_vector: Optional[np.ndarray]=None) -> float:
        ...
        rewards = []
        actions = []
        obs_templates = [{}, {'critical': True}, {'warning': True}, {'resource_level': 0.5}, {'resource_level': 0.1}, {'resource_level': 0.8836}, {'critical': True, 'resource_level': 0.2}]
    ...
    def _real_emergence_detection(self, actions: List[str], rewards: List[float]) -> float:
        ...
        if not emergence_signals:
            return 0.0
        weights = [0.5008, 0.3211, 0.1781][:len(emergence_signals)]
