...
class MetaSME(SelfModificationEngine):
    ...
    def _evaluate_sme_fitness_single(self, sme_source: str, seed: int = 0) -> float:
        """
        单次评估变异后的SME引擎质量（内部方法）

        策略：用变异后的SME运行5代unified_agent改写（快速模式），
    ...
    def _evaluate_sme_fitness(self, sme_source: str, n_runs: int = 3) -> float:
        """
        鲁棒的Meta-fitness评估（多次运行取中位数）

        Args:
            sme_source: 变异后的SME源码
            n_runs: 运行次数（默认3次，取中位数降低方差）

        Returns:
            meta_fitness (0.0 ~ 1.0)，中位数 of n_runs
        """
        fitness_values = []
        ...
        logger.info(
            f"[MetaSME] Meta-fitness (n={n_runs}): "
            f"median={median_fitness:.4f}, mean={mean_fitness:.4f}, std={std_fitness:.4f}, "
            f"values={[round(v, 3) for v in fitness_values]}"
        )
    ...
    def run_meta_evolution(self, max_generations: int = 50) -> Dict:
        ...
        meta_fitness_history = [baseline_meta_fitness]  # 追踪历史用于趋势判断
        ...
        for gen in range(max_generations):
            ...
            if meta_candidates:
                ...
                # 改进的接受策略：放宽阈值，但要求历史趋势向上
                relaxed_threshold = -0.01  # 从 -0.001 放宽到 -0.01
                can_accept = best_meta["delta"] > relaxed_threshold

                # 额外检查：如果历史足够长，要求近期趋势向上
                if can_accept and len(meta_fitness_history) >= 5:
                    recent_values = meta_fitness_history[-5:]
                    trend = np.polyfit(range(5), recent_values, 1)[0]
                    if trend < -0.001:  # 趋势向下，拒绝
                        can_accept = False
                        logger.info(f"[MetaSME] ⚠️  Trend negative ({trend:.4f}), rejecting despite Δ={best_meta['delta']:+.4f}")
                ...
                if can_accept:
                    ...
                    meta_fitness_history.append(baseline_meta_fitness)
                    ...
                    meta_fitness_history.append(baseline_meta_fitness)  # 记录未变化
                    logger.info(
                        f"[MetaSME] ⚠️  Best meta Δ={best_meta['delta']:+.4f} below threshold or trend down"
                    )

            # 早停检查：连续10代无改进
            if generations_without_improvement >= 10:
                logger.info(f"[MetaSME] 🛑 Early stopping: no improvement for {generations_without_improvement} generations")
                break
