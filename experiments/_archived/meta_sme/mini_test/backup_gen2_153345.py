...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1771, 0.4098, 0.1727, 0.2404]), 'concerned': np.array([0.2726, 0.3134, 0.2456, 0.1684]), 'normal': np.array([0.1086, 0.4872, 1.0229, 0.3133]), 'growth': np.array([0.2256, 0.4288, 0.2049, 0.497])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['synthesize', 'explore', 'explore', 'optimize']
