...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.388, 0.1334, 0.2619, 0.1555]), 'concerned': np.array([0.35, 0.35, 0.2, 0.3239]), 'normal': np.array([0.1653, 0.4, 0.211, 0.1]), 'growth': np.array([0.2, 0.1907, 0.513, 0.2])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate']
