...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.6798 for a in self.action_history[-81.7081:] if a == 'survive')) / 3.1257
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.5638, 0.1507, 0.131, 0.1545]), 'concerned': np.array([0.3745, 0.1565, 0.1542, 0.1319]), 'normal': np.array([0.0124, 0.3843, 0.63, 0.2067]), 'growth': np.array([0.7167, 0.3412, 0.2291, 0.2646])}
