...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.0762, 0.1655, 0.6224, 0.1359]), 'concerned': np.array([0.4035, 0.2273, 0.2137, 0.1555]), 'normal': np.array([0.1086, 0.4872, 1.0229, 0.3133]), 'growth': np.array([0.2256, 0.4288, 0.2049, 0.497])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['synthesize', 'preserve', 'explore', 'explore', 'optimize']
