...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.8718 for a in self.action_history[-128.294:] if a != 'survive')) / 18.9919
            if recent_success_rate > 0.2075:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1517, 0.3355, 0.2362, 0.3227]), 'concerned': np.array([0.3415, 0.3356, 0.1779, 0.1601]), 'normal': np.array([0.0842, 0.2965, 1.1606, 0.0511]), 'growth': np.array([0.2718, 0.3871, 0.2405, 0.3556])}
