...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.289 for a in self.action_history[-128.294:] if a == 'survive')) / 13.0359
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1416, 0.1098, 0.1773, 0.5713]), 'concerned': np.array([0.3193, 0.3037, 0.1364, 0.2406]), 'normal': np.array([0.0075, 0.4035, 1.0229, 0.3133]), 'growth': np.array([0.2381, 0.3607, 0.2049, 0.497])}
