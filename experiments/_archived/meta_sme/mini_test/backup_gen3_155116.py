...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.4073 for a in self.action_history[-81.7081:] if a != 'survive')) / 7.6843
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.3085, 0.1991, 0.3147, 0.1776]), 'concerned': np.array([0.3801, 0.1627, 0.2228, 0.01]), 'normal': np.array([0.0707, 0.4269, 0.8109, 0.2854]), 'growth': np.array([0.6063, 0.3019, 0.2128, 0.2808])}
