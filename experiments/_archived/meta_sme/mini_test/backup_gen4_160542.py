...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.7649 for a in self.action_history[-20:] if a != 'survive')) / 20.0
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.5112, 0.0799, 0.2763, 0.1326]), 'concerned': np.array([0.292, 0.3138, 0.2077, 0.2396]), 'normal': np.array([0.238, 0.3522, 0.2528, 0.157]), 'growth': np.array([0.2423, 0.2607, 0.3029, 0.1941])}
