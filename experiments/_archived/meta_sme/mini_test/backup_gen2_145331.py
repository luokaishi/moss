...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1.199 for a in self.action_history[-59.3992:] if a != 'survive')) / 15.7997
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2362, 0.4039, 0.1294, 0.2305]), 'concerned': np.array([0.1537, 0.3809, 0.289, 0.3605]), 'normal': np.array([0.0703, 0.3135, 0.952, 0.01]), 'growth': np.array([0.312, 0.2905, 0.0655, 0.0633])}
