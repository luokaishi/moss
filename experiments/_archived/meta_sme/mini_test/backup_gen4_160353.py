...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.2822 for a in self.action_history[-81.7081:] if a == 'survive')) / 3.5357
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2491, 0.226, 0.1248, 0.4001]), 'concerned': np.array([0.3606, 0.2167, 0.2586, 0.3609]), 'normal': np.array([0.0358, 0.4561, 0.8596, 0.2182]), 'growth': np.array([0.636, 0.4095, 0.2557, 0.2808])}
