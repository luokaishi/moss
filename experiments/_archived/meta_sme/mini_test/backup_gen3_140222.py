...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.25, 0.2989, 0.25, 0.25]), 'concerned': np.array([0.2674, 0.212, 0.1987, 0.3218]), 'normal': np.array([0.1653, 0.1805, 0.3, 0.209]), 'growth': np.array([0.1649, 0.2552, 0.4474, 0.1986])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'influence', 'reflect']
