...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.3618 for a in self.action_history[-70.037:] if a == 'survive')) / 11.4166
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.7, 0.1, 0.1, 0.1]), 'concerned': np.array([0.3756, 0.3975, 0.31, 0.0487]), 'normal': np.array([0.0707, 0.3106, 0.9152, 0.3825]), 'growth': np.array([0.3324, 0.3041, 0.0987, 0.2648])}
