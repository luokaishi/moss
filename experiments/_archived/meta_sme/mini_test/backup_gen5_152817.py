...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.5833 for a in self.action_history[-128.294:] if a == 'survive')) / 12.4266
            if recent_success_rate > 0.0272:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1936, 0.4941, 0.1628, 0.1495]), 'concerned': np.array([0.2916, 0.231, 0.1988, 0.2787]), 'normal': np.array([0.0075, 0.3626, 1.0229, 0.0081]), 'growth': np.array([0.238, 0.3957, 0.2509, 0.497])}
