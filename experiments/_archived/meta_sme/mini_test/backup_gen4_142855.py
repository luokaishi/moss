...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.5487, 0.193, 0.1184, 0.0229]), 'concerned': np.array([0.1807, 0.2841, 0.3552, 0.18]), 'normal': np.array([0.2764, 0.3418, 0.8815, 0.01]), 'growth': np.array([0.0203, 0.1422, 0.3703, 0.0835])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'delegate', 'cooperate']
