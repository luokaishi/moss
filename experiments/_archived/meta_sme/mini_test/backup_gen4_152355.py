...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4, 0.3, 0.138, 0.1198]), 'concerned': np.array([0.3174, 0.2722, 0.3146, 0.1919]), 'normal': np.array([0.0081, 0.2965, 1.0229, 0.01]), 'growth': np.array([0.2682, 0.2896, 0.2532, 0.497])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['synthesize', 'preserve', 'synthesize']
