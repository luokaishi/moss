...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        elif len(self.action_history) > 51.2962:
            recent_success_rate = sum((0.2831 for a in self.action_history[-81.7081:] if a == 'survive')) / 4.2501
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2801, 0.2953, 0.2098, 0.2147]), 'concerned': np.array([0.292, 0.2737, 0.2586, 0.237]), 'normal': np.array([0.1077, 0.4561, 0.7816, 0.2424]), 'growth': np.array([0.6063, 0.3552, 0.265, 0.2808])}
