...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a != 'survive')) / 15.9131
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2764, 0.7246, 0.1533, 0.1]), 'concerned': np.array([0.35, 0.4031, 0.2, 0.1]), 'normal': np.array([0.1353, 0.3819, 0.2284, 0.2543]), 'growth': np.array([0.2, 0.2002, 0.4, 0.1416])}
