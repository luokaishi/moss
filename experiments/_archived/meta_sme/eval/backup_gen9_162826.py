...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.5347 for a in self.action_history[-70.2582:] if a == 'survive')) / 3.5171
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1222, 0.0922, 0.1517, 0.6339]), 'concerned': np.array([0.3795, 0.2315, 0.2517, 0.1373]), 'normal': np.array([0.01, 0.3758, 0.63, 0.2067]), 'growth': np.array([0.6778, 0.2932, 0.3756, 0.2646])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['learn', 'maintain']
