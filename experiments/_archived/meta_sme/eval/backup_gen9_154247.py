...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.3773 for a in self.action_history[-105.4551:] if a == 'survive')) / 13.0359
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.7, 0.1, 0.1, 0.2199]), 'concerned': np.array([0.3756, 0.3975, 0.31, 0.0487]), 'normal': np.array([0.0707, 0.3106, 0.9152, 0.3825]), 'growth': np.array([0.2683, 0.2949, 0.1216, 0.3152])}
