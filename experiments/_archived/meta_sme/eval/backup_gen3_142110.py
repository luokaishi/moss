...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.5752 for a in self.action_history[-24.4221:] if a == 'survive')) / 31.334
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1974, 0.3522, 0.2174, 0.233]), 'concerned': np.array([0.3049, 0.3733, 0.2214, 0.3943]), 'normal': np.array([0.2382, 0.4514, 0.8051, 0.01]), 'growth': np.array([0.0263, 0.144, 0.4577, 0.0564])}
