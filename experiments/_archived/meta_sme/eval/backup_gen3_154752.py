...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2873, 0.2039, 0.2773, 0.2314]), 'concerned': np.array([0.2436, 0.1613, 0.2658, 0.01]), 'normal': np.array([0.0707, 0.3106, 0.7881, 0.3825]), 'growth': np.array([0.4616, 0.3924, 0.2706, 0.2872])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['synthesize', 'explore', 'synthesize', 'learn', 'cooperate']
