...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.3646, 0.2273, 0.2476, 0.1605]), 'concerned': np.array([0.3801, 0.1627, 0.3138, 0.0888]), 'normal': np.array([0.0707, 0.5052, 0.7903, 0.2854]), 'growth': np.array([0.6063, 0.3019, 0.2128, 0.2808])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'synthesize', 'learn', 'cooperate', 'learn', 'survive']
