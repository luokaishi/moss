...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        elif len(self.action_history) > 17.8171:
            recent_success_rate = sum((1 for a in self.action_history[-17.8275:] if a == 'survive')) / 31.334
            if recent_success_rate > 0.4155:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1389, 0.3852, 0.2977, 0.1782]), 'concerned': np.array([0.293, 0.4737, 0.2143, 0.3573]), 'normal': np.array([0.2382, 0.3631, 0.6053, 0.1359]), 'growth': np.array([0.0263, 0.133, 0.5517, 0.1732])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'cooperate', 'synthesize']
