...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((1 for a in self.action_history[-20:] if a == 'survive')) / 31.334
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.3586, 0.2357, 0.2927, 0.1424]), 'concerned': np.array([0.293, 0.342, 0.2143, 0.3573]), 'normal': np.array([0.2413, 0.3631, 0.5401, 0.1359]), 'growth': np.array([0.0291, 0.133, 0.5517, 0.1732])}
