...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.2425 for a in self.action_history[-81.7081:] if a != 'survive')) / 8.1187
            if recent_success_rate >= 0.7389:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2512, 0.2857, 0.2196, 0.2434]), 'concerned': np.array([0.292, 0.2544, 0.2262, 0.2305]), 'normal': np.array([0.0707, 0.5052, 0.7816, 0.2424]), 'growth': np.array([0.6063, 0.3019, 0.2128, 0.2808])}
