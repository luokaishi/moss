...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.7195 for a in self.action_history[-20:] if a != 'survive')) / 24.4812
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.7, 0.0903, 0.1, 0.1]), 'concerned': np.array([0.4086, 0.35, 0.2549, 0.1]), 'normal': np.array([0.162, 0.391, 0.2535, 0.1935]), 'growth': np.array([0.196, 0.2401, 0.2843, 0.2797])}
