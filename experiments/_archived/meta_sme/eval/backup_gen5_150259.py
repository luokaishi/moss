...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.8221 for a in self.action_history[-67.176:] if a == 'survive')) / 14.4648
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1404, 0.1157, 0.5782, 0.1233]), 'concerned': np.array([0.2651, 0.1673, 0.3281, 0.2396]), 'normal': np.array([0.1634, 0.2209, 1.3693, 0.0139]), 'growth': np.array([0.1802, 0.3987, 0.1648, 0.181])}
