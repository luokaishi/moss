...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.7536 for a in self.action_history[-17.8275:] if a == 'survive')) / 31.334
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.2559, 0.3154, 0.2426, 0.1862]), 'concerned': np.array([0.2037, 0.4737, 0.2214, 0.3438]), 'normal': np.array([0.2382, 0.3631, 0.6053, 0.1359]), 'growth': np.array([0.0263, 0.133, 0.4217, 0.0564])}
