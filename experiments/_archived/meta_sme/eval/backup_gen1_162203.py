...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.6798 for a in self.action_history[-74.9769:] if a == 'survive')) / 3.5171
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.138, 0.1441, 0.1426, 0.5753]), 'concerned': np.array([0.4113, 0.1936, 0.1542, 0.1319]), 'normal': np.array([0.0124, 0.3843, 0.63, 0.2067]), 'growth': np.array([0.7167, 0.3412, 0.261, 0.2646])}
