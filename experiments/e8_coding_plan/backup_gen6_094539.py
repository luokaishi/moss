...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.5645, 0.1, 0.2, 0.1]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.2, 0.4, 0.3, 0.1]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'reflect', 'explore', 'adapt', 'innovate', 'evaluate', 'integrate', 'restructure']
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            self.current_state = 'normal'
        # Added diversity boost when in normal state for longer periods
        if self.current_state == 'normal' and len(self.action_history) > 50:
            unique_actions = len(set(self.action_history[-50:]))
            if unique_actions < 5:  # If too few unique actions in last 50 steps
                # Increase exploration by temporarily boosting curiosity weight
                self.weights[1] = min(self.weights[1] + 0.1, 0.8)  # Boost curiosity up to max 80%