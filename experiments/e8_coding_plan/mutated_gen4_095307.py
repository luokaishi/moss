...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            if recent_success_rate > 0.85:  # 提高成功率阈值
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'normal' and len(self.action_history) > 50:
            ...
            if unique_actions < 3:
                self.weights[1] = min(0.6, self.weights[1] + 0.15)  # 增加好奇心权重
                self.weights = self.weights / self.weights.sum()  # 归一化
        # 新增：在危机状态下引入动态权重衰减机制
        elif self.current_state == 'crisis':
            # 如果处于危机状态超过一定步数，逐渐降低非生存维度权重
            if len(self.action_history) > 100 and self.action_history.count('crisis') > 50:
                self.weights[0] = min(0.8, self.weights[0] + 0.05)  # 增加生存权重
                self.weights[1] = max(0.05, self.weights[1] - 0.05)  # 减少好奇心权重
                self.weights = self.weights / self.weights.sum()  # 归一化

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'reflect']
        # 在特定条件下增加新的行动选项以提高多样性
        if len(self.action_history) > 100 and np.random.random() < 0.2:  # 提高新行动出现概率
    ...
    def step(self, observation: Dict=None) -> ActionResult:
        ...
        success = np.random.random() > 0.15  # 调整成功率阈值
        reward = np.random.random() * 0.6 if success else -0.15  # 调整奖励范围
        ...
        if 'valence' in self.dimensions:
            ...
            self.weights = 0.9 * self.weights + 0.1 * valence_w  # 调整学习率
        ...
        if 'norm' in self.dimensions:
            ...
            reward = reward - 0.15 * norm_penalty * abs(reward)  # 调整规范惩罚强度
