...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.4464, 0.1931, 0.1657, 0.1947]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.2, 0.4, 0.3, 0.1]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
    ...
    def _random_action(self) -> str:
        ...
        if len(self.action_history) > 100 and np.random.random() < 0.2:
            actions.extend(['explore', 'analyze', 'adapt'])
