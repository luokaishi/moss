...
class UnifiedMOSSAgent(BaseMOSSAgent):
        ...
        if len(self.action_history) >= 5:
            ...
            if len(unique_actions) <= 2:
                if np.random.random() < 0.35:
                    exploration_actions = ['explore', 'learn', 'create', 'innovate', 'collaborate', 'analyze']
    ...
    def _apply_state_weights(self):
        ...
        if hasattr(self, '_state_streak'):
            if self._state_streak >= 10:
                other_dims = [i for i in range(len(self.weights)) if self.weights[i] < 0.15]
                if other_dims:
    ...
    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'reflect', 'explore', 'adapt', 'learn', 'create', 'innovate', 'collaborate', 'analyze', 'evaluate']
