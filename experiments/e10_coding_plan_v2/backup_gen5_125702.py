...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def select_action(self, observation: Dict) -> str:
        ...
        # 新增：根据环境复杂度调整策略
        env_complexity = observation.get('complexity', 0.5)
        if env_complexity > 0.7:
            # 高复杂度环境下增加探索性行为
            exploration_bias = np.random.random() < 0.3
            if exploration_bias:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1946, 0.1507, 0.0909, 0.5638]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.2, 0.4, 0.3, 0.1]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
        self.weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])
        
        # 新增：根据历史表现动态微调权重
        if len(self.action_history) > 10:
            recent_actions = self.action_history[-10:]
            cooperation_rate = sum(1 for a in recent_actions if a == 'cooperate') / len(recent_actions)
            if cooperation_rate > 0.7:
                # 高合作率时略微提升影响和优化权重
                self.weights[2] += 0.05  # influence
                self.weights[3] += 0.05  # optimization
    ...
    def _exploratory_action(self) -> str:
        """探索性行动 - 在高复杂度环境中使用"""
        exploratory_actions = ['explore', 'analyze', 'test', 'innovate']
        return np.random.choice(exploratory_actions)
...
# MUTATION_INFO: {"strategy": "structure_add", "target