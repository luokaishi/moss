...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if len(self.action_history) >= 10:
            ...
            survival_actions = recent_actions.count('survive')
            curiosity_actions = recent_actions.count('explore')
            
            # If too many survival actions recently, reduce survival weight slightly
            if survival_actions > 6:
                self.weights[0] = max(0.1, self.weights[0] - 0.1)
                # Distribute the reduced weight among other dimensions
                remaining_weight = 0.1
                non_zero_indices = np.where(self.weights > 0)[0]
                if len(non_zero_indices) > 0:
                    additional_per_dim = remaining_weight / len(non_zero_indices)
                    for idx in non_zero_indices:
                        self.weights[idx] += additional_per_dim
    ...
    def _random_action(self) -> str:
        ...
        if np.random.random() < 0.4:
            exploration_actions = ['explore', 'analyze', 'adapt', 'innovate']
...
# MUTATION_INFO: {"strategy": "behavior_shift", "target_function": "_apply_state_weights", "description": "Added dynamic weight adjustment based on recent action history to improve behavioral diversity and adaptation", "confidence":