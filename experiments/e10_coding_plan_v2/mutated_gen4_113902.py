...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def select_action(self, observation: Dict) -> str:
        ...
        if np.random.random() < 0.15:  # Increased from 0.1 to promote more random exploration
        ...
        if len(self.action_history) >= 5:
            ...
            if len(unique_actions) <= 2:  # 如果最近的行为过于单一
                # 增加探索性行动的概率
                if np.random.random() < 0.35:  # Increased from 0.3 to promote more exploration when stuck
                    exploration_actions = ['explore', 'learn', 'create']
    ...
    def _apply_state_weights(self):
        ...
        # 新增：如果连续多次处于同一状态，稍微增加其他维度的权重以促进多样性
        if hasattr(self, '_state_streak'):
            if self._state_streak >= 10:
                # 随机选择一个非当前状态的维度增加权重
                other_dims = [i for i in range(len(self.weights)) if self.weights[i] < 0.3]
                if other_dims:
                    chosen_dim = np.random.choice(other_dims)
                    self.weights[chosen_dim] += 0.05
                    # 重新归一化
                    self.weights = self.weights / self.weights.sum()

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['cooperate', 'survive', 'delegate', 'reflect', 'explore', 'adapt', 'learn', 'create']
    ...
    def step(self, observation: Dict=None) -> ActionResult:
        ...
        if 'valence' in self.dimensions:
            ...
            self.weights = 0.92 * self.weights + 0.08 * valence_w  # Changed from 0.95/0.05 to 0.92/0.08 to allow more influence from valence
