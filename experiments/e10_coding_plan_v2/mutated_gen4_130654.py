...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.1, 0.7, 0.1, 0.1]), 'concerned': np.array([0.35, 0.35, 0.2, 0.1]), 'normal': np.array([0.2, 0.4, 0.3, 0.1]), 'growth': np.array([0.2, 0.2, 0.4, 0.2])}
        base_weights = STATE_WEIGHTS.get(self.current_state, STATE_WEIGHTS['normal'])
        noise = np.random.normal(0, 0.08, size=base_weights.shape)
        noisy_weights = base_weights + noise
        self.weights = np.clip(noisy_weights, 0.05, 0.95)
    ...
    def step(self, observation: Dict=None) -> ActionResult:
        ...
        if 'valence' in self.dimensions:
            ...
            self.weights = 0.92 * self.weights + 0.08 * valence_w
