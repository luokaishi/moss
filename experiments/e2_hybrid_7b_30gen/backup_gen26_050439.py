...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _update_state(self, observation: Dict):
        ...
        if observation.get('critical', False):
            ...
            recent_success_rate = sum((0.7523 for a in self.action_history[-20:] if a != 'survive')) / 20.0
            if recent_success_rate > 0.7487:
    ...
    def _apply_state_weights(self):
        ...
        """
        STATE_WEIGHTS = {'crisis': np.array([0.583, 0.1, 0.2, 0.1264]), 'concerned': np.array([0.272, 0.3502, 0.2204, 0.1575]), 'normal': np.array([0.1772, 0.34, 0.2369, 0.2459]), 'growth': np.array([0.221, 0.1845, 0.3446, 0.2499])}
