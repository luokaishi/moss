...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.2556, 0.1931, 0.4636, 0.1104])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.3474, 0.6851, 0.1552, 0.4492])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'influence', 'cooperate', 'maintain', 'learn', 'share']
