...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state != 'crisis':
            self.weights = np.array([0.1818, 0.0884, 0.2029, 0.5269])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.1097, 0.2617, 0.5181, 0.3995])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['challenge', 'influence', 'influence', 'maintain', 'learn', 'maintain', 'cooperate', 'optimize', 'maintain', 'synthesize', 'learn']
