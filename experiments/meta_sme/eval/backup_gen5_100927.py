...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.1757, 0.2149, 0.1928, 0.2205])
        elif self.current_state == 'concerned':
            self.weights = np.array([0.0983, 0.4851, 0.5134, 0.3572])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['challenge', 'influence', 'cooperate', 'maintain', 'learn', 'share', 'cooperate', 'optimize', 'maintain']
