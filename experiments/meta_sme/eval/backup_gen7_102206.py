...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.3448, 0.2929, 0.1291, 0.1183])
        elif self.current_state == 'concerned':
            self.weights = np.array([0.0917, 0.4579, 0.642, 0.3522])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['influence', 'influence', 'maintain', 'learn', 'maintain', 'cooperate', 'optimize', 'maintain', 'synthesize', 'learn', 'adapt', 'challenge']
