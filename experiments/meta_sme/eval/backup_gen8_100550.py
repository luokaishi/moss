...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.1974, 0.2424, 0.2471, 0.3131])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.3426, 0.7351, 0.158, 0.4634])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'influence', 'cooperate', 'maintain', 'learn', 'share', 'cooperate', 'optimize']
