...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.4155, 0.2455, 0.2211, 0.1178])
        elif self.current_state == 'concerned':
            self.weights = np.array([0.0917, 0.4579, 0.5623, 0.3995])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['challenge', 'influence', 'influence', 'maintain', 'learn', 'maintain', 'cooperate', 'optimize', 'maintain', 'synthesize', 'learn', 'adapt']
