...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state != 'crisis':
            self.weights = np.array([0.2747, 0.2537, 0.0794, 0.3868])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.1895, 0.4851, 0.5134, 0.3792])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['challenge', 'influence', 'influence', 'maintain', 'learn', 'share', 'cooperate', 'optimize', 'maintain', 'synthesize']
