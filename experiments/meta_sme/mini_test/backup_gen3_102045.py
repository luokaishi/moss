...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.2579, 0.2545, 0.2275, 0.2601])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.0917, 0.4579, 0.5623, 0.4987])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['influence', 'influence', 'maintain', 'learn', 'maintain', 'cooperate', 'optimize', 'maintain', 'synthesize', 'learn', 'adapt']
