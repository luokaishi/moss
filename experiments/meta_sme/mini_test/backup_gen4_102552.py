...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.6088, 0.0802, 0.1732, 0.2715])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.1033, 0.4619, 0.6474, 0.1492])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['influence', 'influence', 'maintain', 'learn', 'maintain', 'cooperate', 'optimize', 'maintain', 'synthesize', 'learn', 'adapt', 'challenge', 'explore']
