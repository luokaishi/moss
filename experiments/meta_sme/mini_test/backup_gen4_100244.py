...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.5114, 0.1695, 0.2036, 0.1154])
        elif self.current_state == 'concerned':
            self.weights = np.array([0.3016, 0.5808, 0.1552, 0.4492])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'influence', 'cooperate', 'maintain', 'learn', 'share', 'cooperate']
