...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state != 'crisis':
            self.weights = np.array([0.3235, 0.25, 0.25, 0.25])
        elif self.current_state == 'concerned':
            self.weights = np.array([0.1712, 0.6298, 0.1338, 0.3072])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'survive', 'influence', 'optimize', 'cooperate', 'maintain', 'learn', 'share', 'share']
