...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.0796, 0.1593, 0.7082, 0.0529])
        elif self.current_state == 'concerned':
            self.weights = np.array([0.3112, 0.6298, 0.1338, 0.1605])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'survive', 'influence', 'optimize', 'cooperate', 'maintain', 'learn', 'share']
