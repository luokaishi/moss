...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.5115, 0.2388, 0.1779, 0.0718])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.1556, 0.7107, 0.1338, 0.3059])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'influence', 'optimize', 'cooperate', 'maintain', 'learn', 'share']
