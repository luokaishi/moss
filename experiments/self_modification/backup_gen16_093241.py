...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.3958, 0.173, 0.3083, 0.1228])
        elif self.current_state != 'concerned':
            self.weights = np.array([0.2572, 0.6298, 0.1338, 0.3059])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'survive', 'influence', 'optimize', 'cooperate', 'maintain', 'learn', 'optimize']
