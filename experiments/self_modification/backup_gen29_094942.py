...
class UnifiedMOSSAgent(BaseMOSSAgent):
    ...
    def _apply_state_weights(self):
        ...
        if self.current_state == 'crisis':
            self.weights = np.array([0.3076, 0.2603, 0.1329, 0.2992])
        elif self.current_state == 'concerned':
            self.weights = np.array([0.2572, 0.6613, 0.1338, 0.3059])

    def _random_action(self) -> str:
        """随机行动"""
        actions = ['explore', 'reflect', 'cooperate', 'synthesize', 'learn', 'share']
