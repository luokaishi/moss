#!/usr/bin/env python3
"""
MOSS v8.0 - Coding Plan 10代快速实验
======================================
10代版本，确保在sandbox超时前完成
"""
...
def run_experiment():
    print("="*70)
    print("Experiment: Coding Plan 10gen (Fast)")
    print("="*70)
    ...
    config = SMEConfig(
        ...
        output_dir="experiments/e11_coding_plan_10gen",
    )

    sme = SelfModificationEngine(config=config, hybrid_config=hybrid_config)

    start_time = time.time()
    report = sme.run(max_generations=10, early_stop_fitness=0.90)
    ...
    output_file = f"experiments/experiment_coding_plan_10gen_{time.strftime('%Y%m%d_%H%M%S')}.json"
    with open(output_file, 'w') as f:
        json.dump(result, f, indent=2, default=str)

    print(f"\n{'='*70}")
    print(f"✅ 10-gen experiment completed!")
