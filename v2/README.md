...
**本目录 (`v2/`) = MOSS v2.0.0 = Evo 实验架构**

为避免混淆，请区分以下概念：

| 术语 | 含义 | 位置 |
|------|------|------|
| **MOSS v0.3.0** | 稳定版本（固定权重） | 根目录 `/` |
| **MOSS v2.0.0** | 实验版本（自演化动态权重） | `v2/` 目录（本目录） |
| **Exp-Alpha** | 72小时保守实验（v0.3.0框架） | `experiments/moss_72h_experiment.py` |
| **Exp-Beta** | 72小时激进实验（v0.3.0框架） | `experiments/moss_72h_experiment_v3.py` |

> 保留 `v2/` 目录名是为了避免破坏已发布的 GitHub 链接，语义上等于 `v2.0.0`。

---

## 架构概述

MOSS v2.0.0 (Evo) 是 MOSS 的大版本跃升，从 **固定权重优化** 演进为 **动态权重自演化**。

### 版本定位

| 版本 | 核心特性 | 状态 | 对比 |
|------|----------|------|------|
| **MOSS v0.3.0** | 固定权重多目标优化 | ✅ 稳定 | 基线 |
| **MOSS v2.0.0** | 自修改动态权重演化 | 🔄 实验 | 跃升 |
